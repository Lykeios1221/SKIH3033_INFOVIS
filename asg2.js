...
Map.addLayer(roi.style({
    color: "red", fillColor: "00000000", width: 4,
}), {}, "Perak Boundary");

var visualization = {
    bands: ["B4", "B3", "B2"], min: 0.0, max: 3000, width: 4, color: "red",
};
...
var visualization = {
    bands: ["B4", "B3", "B2"], min: 0.0, max: 3000,
};
...
function getROIImageWithNDRE(year) {
    ...
    var resampledImage = image
        .setDefaultProjection(s2MaskedImageCollection.select(1).first().projection())
        .resample("bicubic");

    Map.addLayer(resampledImage, visualization, "Resampled Sentinel-2 Perak Image in " + year);

    return resampledImage;
}

var dataset = [{
    year: 2018, image: roiImages[0], classData: data2018,
}, {
    year: 2023, image: roiImages[1], classData: data2023,
},];

// Classification visualize args
var viz = {
    min: 0, max: 5, palette: ["#d63000", // Residential
        "#a604ff", // Industry
        "#1488ff", // Water bodies
        "#00d308", // Forest
        "#f3f706", // Vegetation
...
function model(args) {
    ...
    var data = args.image
        .sampleRegions({
            collection: args.classData, properties: ["class"], scale: 30, tileScale: 8,
        })
        .randomColumn({
            seed: 3033,
        });
    ...
    // Build RF model
    var rfClf = ee.Classifier.smileRandomForest({numberOfTrees: 50, seed: 3033}).train(training, "class", bands);
    results.push(evaluateModel(args.image, testing, rfClf, "Random Forest", null));

    // Build GB model
    var gbClf = ee.Classifier.smileGradientTreeBoost({numberOfTrees: 15, seed: 3033}).train(training, "class", bands);
    results.push(evaluateModel(args.image, testing, gbClf, "Gradient Boosting", null));

    var bestModel = ee
        .FeatureCollection(ee.List(results).map(function (dict) {
            dict = ee.Dictionary(dict);
            return ee.Feature(null, {
                model: dict.get("model"), accuracy: dict.getNumber("accuracy"),
            });
        }))
        .sort("accuracy", false)
        .first();
    ...
    var tunedModel = modelTuning(args.year, args.image, bestModel.model, training, testing)
    return tunedModel;
}
...
function evaluateModel(img, testing, clf, name, layerName) {
    ...
    if (layerName) {
        Map.addLayer(classificationResult, viz, layerName)
    }
    // Model testing
    ...
    return {
        model: clf, accuracy: accuracy,
    };
}

var tunedBestModels = dataset.map(model);
...
function modelTuning(year, image, model, training, testing) {
    ...
    var accuracies = numTreesList.map(function (numTrees) {
        var classifier = ee.Classifier.smileRandomForest({numberOfTrees: numTrees, seed: 3033})
            .train({
                features: training, classProperty: 'class', inputProperties: bands
            });

        // Classify the testing data
        var accuracy = testing.classify(classifier)
            .errorMatrix('class', 'classification')
            .accuracy();

        return ee.Feature(null, {
            'accuracy': accuracy, 'numberOfTrees': numTrees
        });
    });
    ...
    // Create chart
    var chart = ui.Chart.feature.byFeature(accuracyFeatures, 'numberOfTrees', 'accuracy')
        .setChartType('LineChart')
        .setOptions({
            title: 'Hyperparameter Tuning for the numberOfTrees Parameter', vAxis: {
                title: 'Validation Accuracy'
            }, hAxis: {
                title: 'Number of Trees', gridlines: {
                    count: 15
                }
            }, lineSize: 3, pointSize: 0, legend: {position: 'none'}
        });

    print(chart);

    // Find the optimal number of trees
    var maxAccuracyFeature = accuracyFeatures.sort('accuracy', false)
        .first();
    ...
    evaluateModel(image, testing, tunedRFClf, 'Tuned Random Forest', 'Land Use Classification of Perak in ' + year + ' by Tuned RF')
}
